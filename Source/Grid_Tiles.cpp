...
Grid_Tiles::Grid_Tiles(SDL_Renderer* ren, SDL_Event* ev, bool* erase, bool* paint) : m_ren(ren), m_ev(ev), m_erase(erase), m_paint(paint) {
	m_mouseClick = false;
	m_activeLayer = -1;
...
bool Grid_Tiles::AddLayer(std::string layerName, int rows, int columns, int Bwidth, int Bheight) {
...
void Grid_Tiles::RemoveLayer(int i) {
	if (m_layers.empty())
		return;
...
Layer& Grid_Tiles::AccessLayer() {
	return m_layers[m_activeLayer];
}
void Grid_Tiles::SetActiveLayer(int i) {
	if (i < 0 || i >= m_layers.size())
		return;
	m_activeLayer = i;
}
void Grid_Tiles::ShowOutline() {
	m_showOutline++;
	if (m_showOutline >= 3)
		m_showOutline = 0;
}
void Grid_Tiles::Save(std::string path) {
	m_folderPath = path;
	Save();
}
void Grid_Tiles::Load(std::string path) {
	m_folderPath = path;
	Load();
}
void Grid_Tiles::Save() {
...
SDL_Surface* Grid_Tiles::GetLayerSurface(SDL_Window* window) {
	if (!m_layers.empty()) {
